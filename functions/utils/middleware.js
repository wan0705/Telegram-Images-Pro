...
export async function errorHandling(context) {
    ...
    if (typeof env.disable_telemetry == "undefined" || env.disable_telemetry == null || env.disable_telemetry == "") {
        ...
        try {
            ...
            if (sampleRate) remoteSampleRate = sampleRate;
    ...
    if (typeof env.disable_telemetry == "undefined" || env.disable_telemetry == null || env.disable_telemetry == "") {
        try {
            ...
            context.request.headers.forEach((value, key) => {
                ...
                if (value.length > 0) context.data.sentry.setTag(key, value);
            });
            const CF = JSON.parse(JSON.stringify(context.request.cf));
            const parsedCF = {};
            for (const key in CF) {
                if (typeof CF[key] == "object") { parsedCF[key] = JSON.stringify(CF[key]); }
                else { parsedCF[key] = CF[key]; if (CF[key].length > 0) context.data.sentry.setTag(key, CF[key]); }
            }
            const data = { headers: parsedHeaders, cf: parsedCF, url: context.request.url, method: context.request.method, redirect: context.request.redirect };
        ...
        finally { if (context.data.transaction) context.data.transaction.finish(); }
    }
    return context.next();
}
...
export async function traceData(context, span, op, name) {
    ...
    if (data.telemetry) {
        if (span) { span.finish(); }
        else { span = await context.data.transaction.startChild({ op: op, name: name }); }
    }
}
